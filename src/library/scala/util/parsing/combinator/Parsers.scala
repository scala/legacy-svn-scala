...
trait Parsers {
  ...
  sealed abstract class ParseResult[+T] {
    ...
    def filterWithError(p: T => Boolean, error: T => String): ParseResult[T]
  ...
  sealed abstract class NoSuccess(val msg: String, override val next: Input) extends ParseResult[Nothing] { // when we don't care about the difference between Failure and Error
    ...
    def filterWithError(p: Nothing => Boolean, error: Nothing => String): ParseResult[Nothing] = this
  ...
  abstract class Parser[+T] extends (Input => ParseResult[T]) {
    ...
    def filter(p: T => Boolean): Parser[T]
      = Parser{ in => this(in) filterWithError(p, "Input doesn't match filter: "+_)}
