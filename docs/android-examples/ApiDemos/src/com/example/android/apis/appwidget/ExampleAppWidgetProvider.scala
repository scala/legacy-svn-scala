 ...
 * </ul>
 */
object ExampleAppWidgetProvider {
  // log tag
  private final val TAG = "ExampleAppWidgetProvider"

  def updateAppWidget(context: Context, appWidgetManager: AppWidgetManager,
                      appWidgetId: Int, titlePrefix: String) {
    Log.d(TAG, "updateAppWidget appWidgetId=" + appWidgetId + " titlePrefix=" + titlePrefix)
    // Getting the string this way allows the string to be localized.  The format
    // string is filled in using java.util.Formatter-style format strings.
    val text = context.getString(R.string.appwidget_text_format,
      ExampleAppWidgetConfigure.loadTitlePref(context, appWidgetId),
      "0x" + SystemClock.elapsedRealtime.toHexString)

    // Construct the RemoteViews object.  It takes the package name (in our case, it's our
    // package, but it needs this because on the other side it's the widget host inflating
    // the layout from our package).
    val views = new RemoteViews(context.getPackageName(), R.layout.appwidget_provider)
...
class ExampleAppWidgetProvider extends AppWidgetProvider {
  import ExampleAppWidgetProvider._ // companion object
  ...
  override def onUpdate(context: Context, appWidgetManager: AppWidgetManager,
    ...
    for (i <-0 until N) {
      ...
      val titlePrefix = ExampleAppWidgetConfigure.loadTitlePref(context, appWidgetId)
