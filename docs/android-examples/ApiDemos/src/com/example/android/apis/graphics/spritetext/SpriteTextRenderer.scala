...
class SpriteTextRenderer(context: Context) extends GLSurfaceView.Renderer {
  ...
  def onSurfaceCreated(gl: GL10, config: EGLConfig) {
    ...
    val is = context.getResources.openRawResource(R.drawable.robot)
    val bitmap: Bitmap = try {
      BitmapFactory.decodeStream(is)
    } finally {
      try { is.close() } catch { case e: IOException => /* Ignore. */ }
    }
