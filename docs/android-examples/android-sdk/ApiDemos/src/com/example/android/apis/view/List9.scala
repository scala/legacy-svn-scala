...
import android.widget.{AbsListView, ArrayAdapter, ListView, TextView}
 ...
 * to display the first letter of the visible range of cheeses.
 */
class List9 extends ListActivity with AbsListView.OnScrollListener {
  ...
  private val mRemoveWindow = new RemoveWindow()
  private val mHandler = new Handler()
  private var mWindowManager: WindowManager = _
  private var mDialogText: TextView = _
  private var mShowing: Boolean = _
  private var mReady: Boolean = _
  private var mPrevLetter = java.lang.Character.MIN_VALUE
  ...
  override def onCreate(savedInstanceState: Bundle) {
    ...
    mHandler post new Runnable() {
      def run() {
        mReady = true
        val lp = new WindowManager.LayoutParams(
  ...
  def onScroll(view: AbsListView, firstVisibleItem: Int,
    ...
    if (mReady) {
      val firstLetter = mStrings(firstVisibleItem) charAt 0

      if (!mShowing && firstLetter != mPrevLetter) {
        mShowing = true
        mDialogText setVisibility View.VISIBLE 
      }
      mDialogText setText firstLetter.toString
