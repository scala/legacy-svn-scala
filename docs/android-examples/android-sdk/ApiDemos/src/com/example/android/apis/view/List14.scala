...
/**
 * Demonstrates how to write an efficient list adapter. The adapter used in
 * this example binds to an ImageView and to a TextView for each row in the list.
 *
 * To work efficiently the adapter implemented here uses two techniques:
 * - It reuses the convertView passed to getView() to avoid inflating View when
 *   it is not necessary
 * - It uses the ViewHolder pattern to avoid calling findViewById() when it is
 *   not necessary
 *
 * The ViewHolder pattern consists in storing a data structure in the tag of
 * the view returned by getView(). This data structures contains references to
 * the views we want to bind data to, thus avoiding calls to findViewById()
 * every time getView() is invoked.
 */
class List14 extends ListActivity {
  import List14._  // companion object
...
object List14 {
  ...
  private class EfficientAdapter(context: Context) extends BaseAdapter {
    import EfficientAdapter._ // companion object

    // Cache the LayoutInflate to avoid asking for a new one each time.
    private val mInflater = LayoutInflater.from(context)
    // Icons bound to the rows.
    private val mIcon1 =
      BitmapFactory.decodeResource(context.getResources, R.drawable.icon48x48_1)
    private val mIcon2 =
      BitmapFactory.decodeResource(context.getResources, R.drawable.icon48x48_2)
    ...
    def getView(position: Int, convertView0: View, parent: ViewGroup): View = {
      ...
      if (convertView == null) {
        ...
        holder.text = convertView.findViewById(R.id.text).asInstanceOf[TextView]
        holder.icon = convertView.findViewById(R.id.icon).asInstanceOf[ImageView]

        convertView setTag holder
      } else {
        // Get the ViewHolder back to get fast access to the TextView
        // and the ImageView.
        holder = convertView.getTag.asInstanceOf[ViewHolder]
      }
