...
import android.widget.{AbsListView, BaseAdapter, ListView, TextView}
 ...
 * flings. In this case, we pretend that binding a view to its data is slow
 * (even though it really isn't). When a scroll/fling is happening, the
 * adapter binds the view to temporary data. After the scroll/fling
 * has finished, the temporary data is replace with the actual data.
 *
 */
class List13 extends ListActivity with OnScrollListener {
  import List13._  // companion object

  private var mStatus: TextView = _
   ...
   * 
   */
  private class SlowAdapter(context: Context) extends BaseAdapter {

    private var mInflater =
    ...
    def getView(position: Int, convertView: View, parent: ViewGroup): View = {
      val text = if (convertView == null) {
        mInflater.inflate(
          android.R.layout.simple_list_item_1, parent, false).asInstanceOf[TextView]
      } else {
  ...
  override def onCreate(savedInstanceState: Bundle) {
    ...
    mStatus = findViewById(R.id.status).asInstanceOf[TextView]
  ...
  def onScrollStateChanged(view: AbsListView, scrollState: Int) {
    scrollState match {
      case OnScrollListener.SCROLL_STATE_IDLE =>
        ...
        val first = view.getFirstVisiblePosition
        val count = view.getChildCount
        for (i <- 0 until count) {
          val t = view.getChildAt(i).asInstanceOf[TextView]
          if (t.getTag != null) {
